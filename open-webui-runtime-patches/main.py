...
LOCAL_FILE_VIEWER_INTERNAL_URL = os.environ.get(
    "LOCAL_FILE_VIEWER_INTERNAL_URL", "http://local-file-viewer:8000"
).rstrip("/")
LOCAL_FILE_PROXY_TIMEOUT_SECONDS = float(
    os.environ.get("LOCAL_FILE_PROXY_TIMEOUT_SECONDS", "30")
)
LOCAL_FILE_HOP_BY_HOP_HEADERS = {
    "connection",
    "keep-alive",
    "proxy-authenticate",
    "proxy-authorization",
...
@app.api_route("/local-file", methods=["GET", "HEAD"])
@app.api_route("/local-file/{proxy_path:path}", methods=["GET", "HEAD"])
async def proxy_local_file(request: Request, proxy_path: str = ""):
    target_path = "/local-file"
    if proxy_path:
        target_path = f"{target_path}/{proxy_path}"

    target_url = f"{LOCAL_FILE_VIEWER_INTERNAL_URL}{target_path}"
    ...
    forward_headers = {
        ...
        if key.lower() not in LOCAL_FILE_HOP_BY_HOP_HEADERS and key.lower() != "host"
    }

    timeout = aiohttp.ClientTimeout(total=LOCAL_FILE_PROXY_TIMEOUT_SECONDS)
    ...
    try:
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.request(
                ...
                response_headers = {
                    ...
                    if key.lower() not in LOCAL_FILE_HOP_BY_HOP_HEADERS
                }
                return Response(
                    content=body,
        ...
        return JSONResponse(
            {"detail": f"Local file viewer unavailable: {exc}"},
            status_code=502,
        )
